...
pub(super) enum FileBatchDeliveryError {
    Cancelled,
    SourceUnavailable(String),
    Retryable(String),
}

impl FileBatchDeliveryError {
    fn is_cancelled(&self) -> bool {
        matches!(self, Self::Cancelled)
    }
}

impl std::fmt::Display for FileBatchDeliveryError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Cancelled => formatter.write_str("cancelled"),
            Self::SourceUnavailable(message) | Self::Retryable(message) => {
...
impl From<String> for FileBatchDeliveryError {
...
pub(super) enum OutgoingBatchFailureDisposition {
    Retry,
    Retire,
}

pub(super) fn outgoing_batch_failure_disposition(
    failures: &[(String, FileBatchDeliveryError)],
) -> OutgoingBatchFailureDisposition {
    if failures
        .iter()
        .any(|(_, error)| matches!(error, FileBatchDeliveryError::SourceUnavailable(_)))
    {
...
pub(super) async fn send_file_batch_to_peers(
    ...
    if no_eligible_peers {
        failures.push((
            "TailSync".to_string(),
            FileBatchDeliveryError::Retryable(
                "No eligible peers are currently available".to_string(),
            ),
        ));
    }
    let failure_disposition = outgoing_batch_failure_disposition(&failures);
    ...
    if let Some(message) = retry_message {
        let should_notify = if failure_disposition == OutgoingBatchFailureDisposition::Retire {
            match sync::remove_outgoing_batch(batch_id) {
                ...
                Err(error) => {
                    warn!("Could not retire outgoing file batch {batch_id_hex}: {error}");
    ...
    if !retired && history_saved && (failures.is_empty() || cancelled) {
...
async fn deliver_prepared_batch_to_peers(
    ...
    while let Some(result) = tasks.join_next().await {
        match result {
            ...
            Ok((_, _, Err(error))) if error.is_cancelled() => {}
            ...
            Err(error) => {
                ...
                failures.push((
                    "TailSync".to_string(),
                    FileBatchDeliveryError::Retryable(error.to_string()),
                ));
            }
        }
    }
...
pub(super) async fn resume_outgoing_batches(
    ...
    for journal in batches {
        ...
        let mut failure_disposition = OutgoingBatchFailureDisposition::Retry;
        if !pending_peers.is_empty() {
            ...
            if !failures.is_empty() {
                failure_disposition = outgoing_batch_failure_disposition(&failures);
                warn!(
...
pub(super) async fn notify_file_batch_error(
    ...
    runtime.notify_file_batch_error(settings, message).await;
}
...
pub(super) fn summarize_file_batch_failures(
    failures: &[(String, FileBatchDeliveryError)],
) -> String {
...
async fn send_validated_batch_to_peer(
    ...
    'restart_batch: loop {
        ...
        for (file_index, prepared_file) in prepared.files.iter().enumerate() {
            ...
            let mut file = match tokio::fs::File::open(&prepared_file.path).await {
                Ok(file) => file,
                Err(error) => {
                    let _ = network::queue_peer_frame(
                        &pool,
                    ...
                    .await;
                    let message =
                        format!("Cannot re-open {}: {error}", prepared_file.path.display());
