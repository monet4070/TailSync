...
#[cfg(test)]
const RELEASE_UPDATE_PUBLIC_KEY: &str = include_str!("updater.pub");
...
fn validate_update_package(bytes: &[u8], expected_version: &str) -> Result<(), String> {
    ...
    tailsync_core::updates::validate_package_metadata(&data, expected_version)
}
...
fn validate_update_package(bytes: &[u8], expected_version: &str) -> Result<(), String> {
    ...
    for entry in entries {
        ...
        return tailsync_core::updates::validate_package_metadata(&data, expected_version);
    }
...
#[cfg(test)]
mod tests {
    use super::{validate_update_package, RELEASE_UPDATE_PUBLIC_KEY, UPDATE_PUBLIC_KEY};
    ...
    fn signed_package_version_must_match_the_release_manifest() {
        ...
        assert!(tailsync_core::updates::validate_package_metadata(metadata, "2.1.0").is_ok());
        let error =
            tailsync_core::updates::validate_package_metadata(metadata, "2.2.0").unwrap_err();
