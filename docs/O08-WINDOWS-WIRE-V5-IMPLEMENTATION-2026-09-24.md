...
- Windows 74 个注册的 Tauri 命令已接入 `StableErrorEnvelope`；`supports_stable_errors=true`。TypeScript 入口统一解码、按中英文消息键呈现；无法分类的源错误安全映射为 `internal_error`，不回传路径或源文本。旧文本错误仍能由客户端兼容处理。macOS 保持 `supports_stable_errors=false`。
...
- 在真实 macOS 上运行 Swift 构建/测试，并完成 macOS 稳定错误命令迁移；Windows 主机上的 macOS Rust 编译不能替代该验收。
