...
v3 发现的两个 P0 都源于同一类根因——**完成事务不是原子的、身份不是持久的**：
...
| §1.A ACK 先于 DB 提交 | ✅ 已由 `fe7e4c6` 修复 | `files_received` 完成 SQLite/文件事务后，`finish_file_batch` 才允许 ACK |
| §1.B 完成批次重放不幂等 | ✅ 已由 `fe7e4c6` 修复 | schema v11 持久回执 + `(batch_id,batch_index)` 唯一约束，重放按设备指纹和 manifest 幂等 |
| §1.C 存储不可用通知风暴 | ✅ 已由 `fe7e4c6` 修复 | 存储检查前置，journal 持久化退避和通知节流 |
| §1.D 恢复循环重复哈希 | ✅ 已由 `fe7e4c6` 控制 | 重试持久化退避；传输进度使用单调上报值 |
| §1.E peer 改名后 journal 卡住 | ✅ 改名场景已由 `fe7e4c6` 修复 | journal 以认证指纹匹配 peer；解除配对后的无目标记录仍按保留期清理 |
| §1.F Windows 文件+路径文本双广播 | ✅ 已由 `fe7e4c6` 修复 | 文件事件处理后显式 `continue`，不再落入文本广播 |
...
- **旧单文件发送路径已删除**：接收端只接受带 batch 的文件协议；两端不再保留无法被调用且一旦复活就会被协议拒绝的 `send_file_to_peers`、专属哈希函数和相关死代码。
...
- v10 迁移显式写入 `10`，v11 分支写入 `SCHEMA_VERSION`；当前 schema 为 v11，不再存在把未来版本写进旧迁移分支的维护陷阱。
...
- Iroh 身份轮换给出明确提示。
- `sync_warning` 测试已用专用锁隔离，并合并重复用例；生产的单一最新 warning 语义不变。
