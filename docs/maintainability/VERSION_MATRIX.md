...
| 13 | `Cargo.lock` | tailsync-core + 3 抽离 crate 条目 | 2.1.0 | ⚠️ 间接 | shared workspace 唯一 lock |
| 14 | `windows/package-lock.json` | root + `packages[""]` | 2.1.0 | ❌ | npm ci 一致性 |
| 15 | `site/package-lock.json` | root + `packages[""]` | 2.1.0 | ❌ | 同上 |
| 16 | `README.md` 徽章/正文 | 文本 | v2.1.0 | ❌ | 文档性，非机械校验 |
...
- FACT: 三个 Cargo.lock（根 workspace + 两个独立 app）中的 `tailsync-core`/抽离 crate 版本条目与 package-lock 的 `version` 字段均实测为 2.1.0（本机核查）。
- FACT: 四个 shared crate 共用根 `Cargo.lock`；macOS/Windows app 被 workspace 排除，各自保留独立 lock。
- INFERENCE: "被校验"缺口 = #9/#10（package.json）与 #14/#15（package-lock）——CI 不检查它们；#16 为文档。

## 2. 现状成本

- FACT: 一次版本升级需同步 15 个文件（#1-#15），其中 3 个 Cargo lock 与 2 个 package-lock 若不同步会导致 `--locked` 构建或 `npm ci` 失败——失败模式是"构建期才暴露"而非"提交前暴露"。
- FACT: 现有 validator 只检测不一致、不提供写入入口；无任何脚本提供 bump 功能。
- INFERENCE: 统一工具的价值：单一入口 + 写入后立即自校验 + lock 同步，把 15 文件的手工操作压缩为 1 条命令。
...
```text
node scripts/bump-version.mjs X.Y.Z            # 写入 15 个文件 + 自校验
...
2. 写 #1-#10（JSON/Cargo.toml 精确字段替换，保留格式与注释）。
3. 同步 3 个 Cargo.lock：仅定向更新 workspace/app 根包的版本条目，不重新解析依赖。
4. 写 #14/#15：package-lock 的 root 与 `packages[""]` 两处。
...
- FACT: 四个 shared crate 已收敛为根 workspace，子 crate lock 已删除。
...
- **`scripts/bump-version.mjs` 已实现**（候选 B，Node 脚本，与 validator 同栈）：
  - `node scripts/bump-version.mjs --target X.Y.Z [--root PATH]` —— 写 15 个文件（JSON 解析重写保序 + Cargo.toml [package] 行级替换 + Cargo.lock [[package]] 块级定向替换，不触碰依赖解析）+ 自校验（复用 validate-release-version.mjs）。
  - `--check` —— 只校验（validator + 锁文件一致性 grep），CI 可复用；不一致非零退出。
  - `--dry-run` —— 只报告将写入的文件数，零写入。
  - `--target` 语义版本校验复用 validator 的 tag 正则（`^v?(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)$`）。
- **验收实测（全部 PASS）**：
  - `bump-version.test.mjs` 4 用例（15 文件全写 + 幂等、dry-run 零写、verify 新旧版本正负、锁文件失同步检测）。
  - 真实仓库：`--check --target 2.1.0` PASS（stable channel）；`--check --target 2.2.0` FAIL（负向，exit 1）；`--dry-run --target 2.2.0` 报告 15 文件且零写入。
