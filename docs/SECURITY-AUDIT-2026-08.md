...
| UDP discovery responder replies to every matching probe without per-source throttling | Accepted, next iteration. The response leaks only hostname/port, but a bounded per-source budget plus a global cap is cheap hardening. Platform rules stay shared in core. |
| `repeated_rtt_probes` iroh test ignored since 2026-08-14 | Accepted, next iteration. Resolution path: run it in clean CI repeatedly; un-ignore if stable, otherwise extract the invariants into deterministic tests and move the QUIC test to a scheduled workflow with an owner and review date. |
