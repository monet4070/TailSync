...
| `repeated_rtt_probes` iroh test ignored since 2026-08-14 | Resolved 2026-08-27 and strengthened 2026-08-31: the `#[ignore]` was removed; RTT probes reuse the application's long-lived Endpoint and close only their dedicated RTT-ALPN QUIC connection. The regression test pins the source Endpoint ID, proves an existing business stream survives a reverse-direction probe, and drains each probe before the next attempt. Iroh connects carry a 10 s outer timeout so cold path discovery is not misreported as failure. |
| No dependency-update automation or RustSec audit | Resolved 2026-08-27 and refined 2026-08-28. Dependabot security updates remain enabled for all supported ecosystems. Ordinary version-update pull requests are disabled for Cargo, npm, and GitHub Actions; maintenance upgrades are planned events under the policy in `docs/DEPENDENCIES.md`, pinned by `scripts/check-dependabot-policy.rb`. Cargo upgrades are handled atomically across all three lockfiles with `scripts/update-cargo-locks.sh`. Daily cargo-deny remains the independent RustSec and yanked-dependency gate, running `--locked` over the supported macOS/Windows graphs; the three lockfiles use patched `h2` 0.4.19. Advisory exceptions require a full registry entry (reason, upstream link, owner, expiry, tracking issue) validated by `scripts/check-rustsec-exceptions.mjs`, which fails on invalid dates, duplicates, drift, or expiry. The registry is currently empty: source and compiler checks proved `bincode` was an unused manifest residue, so it was removed instead of migrated. Transitive unmaintained notices remain owned by their direct-dependency upgrade path. Licenses/bans/sources checks are phase 2. The iroh `=1.1.0` pin carries its review rationale in `Cargo.toml`. |
