...
| No dependency-update automation or RustSec audit | Resolved 2026-08-27: monthly Dependabot across all three Cargo workspaces, both npm projects, and Actions (no auto-merge, no grouping — high-risk deps get individual PRs); cargo-deny advisories checks run on push/PR over a three-manifest matrix and daily via a scheduled workflow; advisory exceptions require a full registry entry (reason, upstream link, owner, expiry, tracking issue) validated by `scripts/check-rustsec-exceptions.mjs`, which fails the day an exception expires. Licenses/bans/sources checks are phase 2. The iroh `=1.0.3` pin now carries its rationale in `Cargo.toml`. |
