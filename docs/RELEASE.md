...
应写入 `shared/updater.pub` 和两个 `tauri.conf.json`。公钥会随客户端编译并公开分发，
不需要作为 Secret 保存。GitHub Actions 只需要以下两个 Secret：
...
旧的 `TAILSYNC_UPDATER_PUBLIC_KEY` Secret 不再被工作流读取，可以删除；保留也不会参与构建。
...
发布构建缺少仓库中的 updater 公钥或私钥都会直接失败，不会静默生成一个无法验证更新的包。
公钥会编译进两个客户端。轮换公钥需要先用旧私钥签一个
同时信任新公钥的过渡版本，不能直接覆盖。
