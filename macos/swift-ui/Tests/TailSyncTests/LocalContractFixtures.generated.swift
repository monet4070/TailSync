...
func validateGeneratedFixture(_ contract: String, data: Data) throws {
  ...
  case "StableErrorCode": _ = try JSONDecoder().decode(ContractStableErrorCode.self, from: data)
  case "StableErrorDetailClass": _ = try JSONDecoder().decode(ContractStableErrorDetailClass.self, from: data)
  case "StableErrorEnvelope": _ = try JSONDecoder().decode(ContractStableErrorEnvelope.self, from: data)
