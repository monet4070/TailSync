...
impl SecureConnection {
    pub async fn read_frame(&mut self) -> Result<Frame, ProtocolError> {
        loop {
            if let Some(total_size) = self.pending_frame_size()? {
    ...
    fn pending_frame_size(&self) -> Result<Option<usize>, ProtocolError> {
...
pub async fn write_ready(
    ...
    secure
        .write_frame(&Frame::new(Command::HandshakeReady, 0, 0, Vec::new()))
        .await
}
...
async fn write_plain_frame(
    ...
    stream
        .write_all(&Frame::new(command, 0, 0, payload.to_vec()).encode())
        .await?;
    stream.flush().await?;
    Ok(())
}
...
mod tests {
    ...
    async fn encrypted_transport_fragments_a_full_file_chunk() {
        ...
        client
            .write_frame(&Frame::new(Command::FileChunk, 0, 1, chunk.clone()))
            .await
            .unwrap();
